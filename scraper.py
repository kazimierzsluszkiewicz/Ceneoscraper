...
lang_from = 'pl'
lang_to = 'en'
translator = Translator(lang_from,  lang_to)
...
while url:
    ...
    if response.status_code == codes['ok']:
        ...
        for opinion in opinions:
            ...
            single_opinion['content'] = " ".join(single_opinion['content'].re.sub(r"\s+", ' ',single_opinion['content'], flags=re.UNICODE).split(" "))
            single_opinion['content_en'] = translator.translate(single_opinion['content'][500])
            single_opinion['pros_en'] = None if not single_opinion['pros'] else translator.translate(single_opinion['pros'])
            single_opinion['cons_en'] = None if not single_opinion['cons'] else translator.translate(single_opinion['cons'])
